...
legacy_right='bindd = $mainMod, J, split horizontally (next window opens to the right), exec, $scriptsDir/window-layout.sh split-horizontal'
legacy_down='bindd = $mainMod SHIFT, V, split vertically (next window opens below), exec, $scriptsDir/window-layout.sh split-vertical'
...
test "$(grep -Fxc -- "$legacy_right" "$legacy_binds")" -eq 1 ||
  fail 'legacy SUPER+J must dispatch through the layout helper exactly once'
test "$(grep -Fxc -- "$legacy_down" "$legacy_binds")" -eq 1 ||
  fail 'legacy SUPER+SHIFT+V must dispatch through the layout helper exactly once'
...
lua_right='exec(mod .. " + J", "split horizontally (next window opens to the right)", window_layout .. " split-horizontal")'
lua_down='exec(mod .. " + SHIFT + V", "split vertically (next window opens below)", window_layout .. " split-vertical")'
...
test "$(grep -Fxc -- "$lua_right" "$lua_binds")" -eq 1 ||
  fail 'Lua SUPER+J must dispatch through the layout helper exactly once'
test "$(grep -Fxc -- "$lua_down" "$lua_binds")" -eq 1 ||
  fail 'Lua SUPER+SHIFT+V must dispatch through the layout helper exactly once'
